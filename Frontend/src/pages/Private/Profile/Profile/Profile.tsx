...
export function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const response = await api.get(`${auth?.role.toLowerCase()}/${auth?.id}`, { signal: abortController.signal });
        ...
        <div className={styles.profileContainer}>
          <div className={styles.profileHeader}>
            ...
            <p className={styles.userCity}>
              {user.street} {user.streetNumber}, {user.district},
              CEP {user.cep}, {user.city} - {user.uf}
            </p>
